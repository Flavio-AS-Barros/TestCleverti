...
namespace TesteCleverti.Services
{
    public class TodoDbService
    {
        private readonly IMongoCollection<Todo> _todos;

        public TodoDbService(ITodoDatabaseSettings settings)
        {
        ...
        public Todo Get(string id) =>
            _todos.Find<Todo>(todo => todo.TodoId == id).FirstOrDefault();
        ...
        public void Update(string id, Todo todoIn) =>
            _todos.ReplaceOne(todo => todo.TodoId == id, todoIn);

        public void Remove(Todo todoIn) =>
            _todos.DeleteOne(todo => todo.TodoId == todoIn.TodoId);

        public void Remove(string id) =>
            _todos.DeleteOne(todo => todo.TodoId == id);
